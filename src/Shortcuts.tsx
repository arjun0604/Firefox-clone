...
export default function Shortcuts() {
    ...
    const firstRowShortcuts = shortcuts.slice(0, 5);
    const remainingShortcuts = rows > 1 ? shortcuts.slice(5, 5 + (rows - 1) * 6) : [];
    ...
    return (
        <div className="flex justify-center mt-4 px-4">
            <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-4 lg:gap-6 w-full max-w-5xl justify-items-center">
                ...
                {remainingShortcuts.map((s, idx) => (
                    <Shortcut key={idx + 5} name={s.name} url={s.url} img={s.img} onDelete={() => onDelete(s.name)} onEdit={() => onEdit(s.name)} />
                ))}
            </div>
            ...
            <Dialog isOpen={open} onOpenChange={setOpen} className="sm:max-w-106.25 bg-[#2B2A3A] text-white border-gray-800">
                ...
                <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <Input id="add-name" value={newShortcut.name} onChange={(e) => setNewShortcut({ ...newShortcut, name: e.target.value })} className="col-span-3 bg-[#1C1B22] border-gray-700 text-white placeholder:text-white-300" placeholder="e.g. GitHub"  />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="add-url" className="text-right">Site URL</Label>
                        <Input id="add-url" value={newShortcut.url} onChange={(e) => setNewShortcut({ ...newShortcut, url: e.target.value })} className="col-span-3 bg-[#1C1B22] border-gray-700 text-white placeholder:text-white-300" placeholder="https://www.github.com" />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="add-img" className="text-right">Image URL</Label>
                        <Input id="add-img" value={newShortcut.img} onChange={(e) => setNewShortcut({ ...newShortcut, img: e.target.value })} className="col-span-3 bg-[#1C1B22] border-gray-700 text-white placeholder:text-white-300" placeholder="https://..." />
                    </div>
                </div>
                <DialogFooter>
